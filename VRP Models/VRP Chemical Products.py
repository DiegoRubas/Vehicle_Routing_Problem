...
for t in vehicle_numbers:
    ...
    if total_price > 0:
        ...
        # a, b = [], []
        for t_r in truck_routes:
            # if t_r[0] == t and truck_route_bin[t_r].varValue is not None and truck_route_bin[t_r].varValue == 1:
            #     a.append(t_r[1])
        ...
        #     first = a[i] if i < len(a) else "N/A"
        #     second = b[i] if i < len(b) else "N/A"
        #     print("{} : {}".format(first, second))
        # acid = truck_acid[t].varValue == 1
        # base = truck_base[t].varValue == 1
        # print("Truck number {} delivering acid and base: {} and {}".format(t + 1, acid, base))
...
gas_vector = [truck_route_vars[tr] * calc_dist(tr[1]) * gas_rate
              for tr in truck_routes]
temp_bin_vector = [truck_route_bin[tr] * 40000 for tr in truck_routes]
...
for combo in rest_combos:
    ...
    for r in combo:
        ...
        if current_delivery < min_delivery:
            min_delivery = current_delivery
    if a == b == c == g == h == 1 and max_delivery <= truck_max_cap and min_delivery >= truck_min_cap:
        rest_valid_combos.append(combo)
